/**
 * BFF: Verify password reset (set new password with token).
 */

import { resetPasswordSchema } from "~/schema/zod/authSchemas";
...
export default defineEventHandler(async (event) => {
  ...
  const path = getPath("auth", "email_password", "passwordResetVerify");
  ...
  if (!response.ok) {
    const message = (data && typeof data.error === "string") ? data.error : "Password reset failed";
