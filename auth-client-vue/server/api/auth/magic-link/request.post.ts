/**
 * BFF: Magic link request – validate email, forward to auth-core send.
 */

import { magicLinkSchema } from "~/schema/zod/authSchemas";
...
export default defineEventHandler(async (event) => {
  ...
  const path = getPath("auth", "magic_link", "send");
  ...
  if (!response.ok) {
    const message = (data && typeof data.error === "string") ? data.error : "Send magic link failed";
